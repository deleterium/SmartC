...
describe('Optimizations level 2', () => {
    ...
    it('should compile: get_B1 + set_B1', () => {
    ...
    it('should compile: psh+pop same register', () => {
        const code = '#pragma optimizationLevel 2\n asm {INC @a\nPSH $r0\nPOP @r0\nPSH $a\nPOP @a}'
        const assembly = '^declare r0\n^declare r1\n^declare r2\n\nINC @a\nPSH $a\nPOP @a\nFIN\n'
        const compiler = new SmartC({ language: 'C', sourceCode: code })
        compiler.compile()
    ...
    it('should compile without optimization: Dumb endless loop shall not hang optimizer', () => {
        const code = '#pragma optimizationLevel 2\n long a; while (a == 0) a++; while (true);'
        const assembly = '^declare r0\n^declare r1\n^declare r2\n^declare a\n\n__loop1_continue:\nBNZ $a :__loop2_continue\nINC @a\nJMP :__loop1_continue\n__loop2_continue:\nJMP :__loop2_continue\n'
        const compiler = new SmartC({ language: 'C', sourceCode: code })
        compiler.compile()
        expect(compiler.getAssemblyCode()).toBe(assembly)
    })
    it('should optimize: MDV+DIV=DIV', () => {
        const code = '#pragma optimizationLevel 2\n fixed fa, fb; long lc, ld; ld = (fa / fb);'
        const assembly = '^declare r0\n^declare r1\n^declare r2\n^declare f100000000\n^const SET @f100000000 #0000000005f5e100\n^declare fa\n^declare fb\n^declare lc\n^declare ld\n\nSET @ld $fa\nDIV @ld $fb\nFIN\n'
        const compiler = new SmartC({ language: 'C', sourceCode: code })
        compiler.compile()
        expect(compiler.getAssemblyCode()).toBe(assembly)
    })
    it('should optimize: MUL+MDV=MUL', () => {
        const code = 'fixed fa, fb; long lc, ld;\n #pragma optimizationLevel 2\n asm {\n SET @r0 $fa\n MUL @r0 $f100000000\n MDV @r0 $ld $f100000000\n }'
        const assembly = '^declare r0\n^declare r1\n^declare r2\n^declare f100000000\n^const SET @f100000000 #0000000005f5e100\n^declare fa\n^declare fb\n^declare lc\n^declare ld\n\nSET @r0 $fa\nMUL @r0 $ld\nFIN\n'
        const compiler = new SmartC({ language: 'C', sourceCode: code })
        compiler.compile()
        expect(compiler.getAssemblyCode()).toBe(assembly)
...
describe('Optimizations level 3', () => {
    it('should compile: simple test', () => {
        const code = '#pragma optimizationLevel 3\nlong txid; while ((txid = getNextTx()) != 0) { if (getSender(txid) != 0xfefe) continue; }'
