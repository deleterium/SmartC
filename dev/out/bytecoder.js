...
function bytecode(assemblySourceCode) {
    const opCodeTable = [
    ...
    function process(parts, instruction) {
        function getMemoryAddress(varName) {
     ...
     * be restarted.
     */
    function checkBranches(currItem, idx) {
        if (currItem.branchLabel !== undefined) {
            ...
            if (offset < -128 || offset > 127) {
                ...
                switch (currItem.instructionValues[0].value) {
                    case 0x1bn:
                        currItem.instructionValues[0].value = 0x1en;
                        break; // BZR_DAT -> BNZ_DAT
                    case 0x1en:
                        currItem.instructionValues[0].value = 0x1bn;
                        break; // BNZ_DAT -> BZR_DAT
                    case 0x1fn:
                        currItem.instructionValues[0].value = 0x22n;
                        break; // BGT_DAT -> BLE_DAT
                    case 0x22n:
                        currItem.instructionValues[0].value = 0x1fn;
                        break; // BLE_DAT -> BGT_DAT
                    case 0x21n:
                        currItem.instructionValues[0].value = 0x20n;
                        break; // BGE_DAT -> BLT_DAT
                    case 0x20n:
                        currItem.instructionValues[0].value = 0x21n;
                        break; // BLT_DAT -> BGE_DAT
                    case 0x23n:
                        currItem.instructionValues[0].value = 0x24n;
                        break; // BEQ_DAT -> BNE_DAT
                    case 0x24n:
                        currItem.instructionValues[0].value = 0x23n;
                        break; // BNE_DAT -> BEQ_DAT
                }
     ...
     */
    function fillJumpsAndBranches(currItem) {
        if (currItem.branchLabel !== undefined) {
            const search = AsmObj.labels.find(obj => obj.label === currItem.branchLabel);
            if (search === undefined) {
     ...
     */
    function number2hexstring(value, valType) {
        if (valType === 'O') {
