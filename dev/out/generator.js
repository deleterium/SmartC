...
function generate(Program) {
    ...
    */
    function codeGenerator(cgAST, jumpTarget, jumpNotTarget, isReversedLogic) {
                ...
                case 'lookupASN':
                    ...
                    objTree.modifiers.forEach(CurrentModifier => {
                        if (CurrentModifier.type === 'MemberByRef') {
                            const TypeD = Program.typesDefinitions.find(obj => obj.type === 'struct' && obj.name === retMemObj.typeDefinition);
                            if (TypeD === undefined) {
                        ...
                        if (CurrentModifier.type === 'Array') {
                            ...
                            let TypeD; // = Program.typesDefinitions.find( obj => obj.type==="array" && obj.name===objTree.Token.value );
                            if (retMemObj.typeDefinition === undefined) { // array of structs
                                TypeD = Program.typesDefinitions.find(obj => obj.type === 'array' && obj.name === retMemObj.name);
                            }
                            else if (objTree.Token.value === retMemObj.name) { // array simple
                                TypeD = Program.typesDefinitions.find(obj => obj.type === 'array' && obj.name === retMemObj.typeDefinition);
                            }
                            else { // array inside struct
                                TypeD = Program.typesDefinitions.find(obj => obj.type === 'array' && obj.name.indexOf('_' + retMemObj.typeDefinition) > 0);
                            }
                            if (TypeD === undefined) {
                            ...
                            if (retMemObj.declaration.indexOf('_ptr') > 0) {
                                retMemObj.declaration = retMemObj.declaration.slice(0, -4);
                            }
                    ...
                    if (objTree.Operation.type === 'UnaryOperator') {
                        ...
                        if (objTree.Operation.value === '*') { // unary star -> pointer operation
                            ...
                            if (CGenObj.MemObj.declaration.indexOf('_ptr') === -1) {
                                ...
                                CGenObj.MemObj.declaration = (CGenObj.MemObj.declaration + '_ptr');
                            }
        ...
        */
        function moldMemory(ParamMemObj, line) {
            let RetObj;
            let retInstructions = '';
            let retIsNew = false;
